...
async function sendLaharConversion(data: PupilensKeroFormFields): Promise<boolean> {
...
const contactForm = document.getElementById('pp-ko-form') as HTMLFormElement;

if (contactForm) {
    contactForm.addEventListener('submit', (e: Event) => {
        e.preventDefault();
        void (async () => {
            const formData: PupilensKeroFormFields = {
...
// Form validation
function validateForm(data: PupilensKeroFormFields): boolean {
    if (!data.name.trim()) {
...
document.addEventListener('DOMContentLoaded', () => {
    const animatedElements = document.querySelectorAll('.benefit-card, .testimonial-card, .testimonial-video-card, .product-card, .faq-item, .workflow-item');
